...
export default function getBearerToken() {
  ...
  const decrptedKey = forge.pki.decryptRsaPrivateKey(PRIVATE_KEY, "VSe1ymt$d9S")
  console.log("decrypted key", decrptedKey);
  const privateKey = forge.pki.privateKeyFromPem(decrptedKey.trim());
  ...
  const publicKeyFingerprint = PUBLIC_KEY_FINGERPRINT;
  const payload = {
